...
# ================= FORMAT PESAN (DENGAN BLOCKQUOTE & ESCAPING) =================
def format_otp_message(otp_data: Dict[str, Any]) -> str:
    """Format pesan sesuai permintaan user ZuraStore dengan Full Message Blockquote."""
    ...
    # Escape HTML agar karakter <#> tidak merusak parse_mode Telegram
    raw_msg_escaped = html.escape(otp_data.get('raw_message', 'N/A'))
    ...
    return (
        ...
        f"🔐 OTP: <code>{otp}</code>\n\n"
        f"<b>FULL MESSAGE:</b>\n"
        f"<blockquote>{raw_msg_escaped}</blockquote>"
    )
...
<i>Bot is running in Real-time Combo Mode</i>"""
...
# ================= Scraper & Monitor Class (COMBO REALTIME) =================
...
class SMSMonitor:
    ...
    async def initialize(self, p_instance):
        ...
        # Buka tab baru sesuai permintaan
        self.page = await context.new_page()
        print("✅ Playwright page connected successfully in New Tab.")

    async def check_url_login_status(self) -> bool:
        if not self.page: return False
        try:
            current_url = self.page.url
            self.is_logged_in = "mdashboard" in current_url
    ...
    async def fetch_sms(self) -> List[Dict[str, Any]]:
        """Mengambil SMS melalui intersepsi API info agar data utuh (Real-time)."""
        if not self.page or not self.is_logged_in: return []
        messages = []
        try:
            # Cegat respons JSON dari API server agar pesan tidak terpotong oleh HTML
            async with self.page.expect_response(lambda r: "/getnum/info" in r.url, timeout=8000) as resp_info:
                # Trigger refresh internal tabel dengan klik header (Ajax)
                ...
                for item in numbers:
                    if item.get('status') == 'success' and item.get('message'):
                        raw_message_full = item.get('message')
...
# ================= COMBO DETECTION FUNCTION =================

async def wait_for_realtime_change(page):
    """Mendeteksi perubahan tampilan web secara instan menggunakan MutationObserver."""
    try:
...
def check_cmd(stats):
    ...
    try:
        ...
        for u in upd.get("result",[]):
            ...
            elif text == "/startnew": BOT_STATUS["monitoring_active"] = True; send_tg("▶️ Monitoring Started.", target_chat_id=chat_id)
...
async def monitor_sms_loop():
    ...
    async with async_playwright() as p:
        ...
        send_tg("✅ <b>BOT ZURA ACTIVE (REAL-TIME MODE)</b>\nUse <code>/login</code> then <code>/startnew</code>", target_chat_id=ADMIN_ID)
        while True:
            # Daily Refresh at 07:01 WIB
            ...
            try:
                ...
                if BOT_STATUS["monitoring_active"] and monitor.is_logged_in:
                    # 1. SCRAPER DETECT: Tunggu perubahan realtime di DOM
                    changed = await wait_for_realtime_change(monitor.page)
                    
                    # 2. DATA REFRESH: Jika ada SMS masuk, langsung ambil via API
                    if changed:
...
app = Flask(__name__)
@app.route('/')
def index(): return "Zura SMS Monitor API Running"
